...
export class HttpExtensionModel {
   ...
   */
  baseFilter(entities: any[], queryParams: QueryParamsModel, filterField: string[] = []): QueryResultsModel {
    // Filtration
    let entitiesResult = this.searchInArray(entities, queryParams.filter, filterField);
   ...
   * @param incomingArr: any[]
   * @param sortField: string
   * @param sortOrder: string
   */
  sortArray(incomingArr: any[], sortField: string = '', sortOrder: string = 'asc'): any[] {
    if (!sortField) {
      return incomingArr;
    }

    let result: any[];
    result = incomingArr.sort((a, b) => {
    ...
    Object.keys(queryObj).forEach(key => {
      ...
      if (key && !filtrationFields.some(e => e === key) && searchText) {
        ...
        try {
          incomingArray.forEach((element, index) => {
            if (element[key]) {
              const value = element[key].toString().trim().toLowerCase();
              if (value.indexOf(searchText) > -1 && indexes.indexOf(index) === -1) {
                indexes.push(index);
              }
            }
          });

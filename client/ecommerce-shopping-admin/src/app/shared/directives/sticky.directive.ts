...
export class StickyDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  private resizeThrottled$: Observable<void | any[]>;
  ...
  constructor(private stickyElement: ElementRef, @Inject(PLATFORM_ID) private platformId: string) {
    ...
    this.status$ = combineLatest(
      ...
      .pipe(
        ...
        map(
          ([enabled, pageYOffset, marginTop, marginBottom]) => this.determineStatus(
  ...
  private determineStatus(
    ...
    const reachedLowerEdge
      = this.boundaryElement && window.pageYOffset + stickyElementHeight + marginBottom >= (stickyPositions.bottomBoundary - marginTop);
    return {
      isSticky: enabled && pageYOffset > stickyPositions.offsetY,
      reachedLowerEdge,
      marginBottom,
      marginTop,
    };
  ...
  private makeSticky(boundaryReached: boolean = false, marginTop: number, marginBottom: number): void {
    ...
    const offSet
      = boundaryReached ? (this.getComputedStyle(this.boundaryElement).bottom - height - this.marginBottom$.value) : this.marginTop$.value;
